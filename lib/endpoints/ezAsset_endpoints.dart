...
class EzAssetEndpoints {
  ...
  Future<BaseResponse<EzAsset>> getGet(String identifier) async {
    ...
    var baseResponse = BaseResponse.fromApi<EzAsset>(
        response, (response) => EzAsset.fromJson(response));

    return baseResponse;
  }

  Future<BaseResponse<EzAsset>> setSet(
      FormData formData) async {
    var response = await getIt<BaseHttpService>().postFetch("EzAsset/Set", formData, true);
    var baseResponse = BaseResponse.fromApi<EzAsset>(
        response,
        (response) => EzAsset.fromJson(response));

    return baseResponse;
  }

  Future<BaseResponse<EzAsset>> setTagSet(
      FormData formData) async {
